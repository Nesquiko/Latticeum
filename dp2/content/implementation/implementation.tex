...
% in the beginning mention that latticefold (not latticefold+) is used, because
% as of designing and writing this work latticefold+ library wasn't ready.

% disclaimer which will go away in final work, only the IVC step commitment constrains
% and not all RISC-V instruction are implemented. Need to add the folding proof
% verification, wrapping snark of the last folding + memory consistency check.

% AFTER ALL ELSE IS READY, CREATE SOME SORT OF INTRO TO THIS CHAPTER, BUT NOT NOW!!!
...
% CCS building
% 	- briefly describe how the ccs is built
%		- I can add new vars and just edit the CCS layout and how the witness is created, all other indices are shifted accordingly
%	- example: 1 sponge pass number of constrains and how does CCS building look like?

\section{Circuit synthesis}\label

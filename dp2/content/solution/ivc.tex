...
authenticated history of the computation terminating at step $i-1$. This
...
witness. Then after recomputing it, a constraint asserts that the recomputed
candidate $h'_{i-1}$ is equal to the provided public input $h_{i-1}$.

This mechanism ensures that the prover cannot forge a history or substitute
arbitrary accumulators. They must possess the valid preimage data
corresponding to the public commitment.
...
state tuple $z_i$ does not comprise of only raw values, but a of more cryptographic
commitments to a complex machine state. If the internal structure of $z_i$'s
preimage is not constrained within the circuit, a malicious prover could
substitute the state commitment $z_{i-1\_\text{comm}}$ for an arbitrary value
that is disconnected from the actual ZkVM execution history.

To prevent such attacks and ensure the integrity of the virtual machine state
transitions, the augmented circuit $F'$ must enforce constraints on the
specific components of the $z_i$ preimage:
...
\begin{enumerate}
    ...
    \item \textbf{Memory pages:} The VM memory is represented by a Merkle
		...
		$z_{i-1\_\text{comm}}$. It then calculates the new root resulting from
		the memory update and constrains that this new root matches the memory
		component in the preimage of $z_{i\_\text{comm}}$.
    ...
    \item \textbf{Register state:} The state includes a Poseidon2 commitment
		...
		register component of the $z_{i\_\text{comm}}$ preimage.
...
LatticeFold Non Interactive Folding Scheme (NIFS). This component effectively
verifies the mathematical correctness of the folding process without re
executing the prior computation.

The logic branches based on the step index:

\begin{itemize}
    \item \textbf{Base case ($i=0$):} The circuit verifies that the step
		counter is zero and that the accumulator is in its initial empty
		state. No folding proof verification occurs.
    \item \textbf{Recursive case ($i>0$):} The circuit accepts the folding
		proof $\pi_{i-1}$ as part of the witness. It then executes the
		LatticeFold verifier with the previous accumulator $U_{i-1}$ and proof
		$\pi_{i-1}$, both taken from the private witness.
\end{itemize}
...
subsequent step. This includes the new folded accumulator instance and the new
folding proof. Finally, the circuit computes the new IVC commitment $h_i$ by

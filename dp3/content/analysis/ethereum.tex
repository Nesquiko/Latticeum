...
(one epoch) attest to that new block's validity. In order for a block to be
...
transactions\footnote{Transaction and state-transition validity are verified by
	execution clients; consensus clients coordinate agreement on blocks containing execution payloads \ref{subsec:ethereum_consensus}.},
outputs next valid state:

\[
	F(state_n, T) = state_{n+1} \label{eq:state_transition}
\]

...
independent basis vectors $B = \{\mathbf{b}_1, \mathbf{b}_2, \dots, \mathbf{b}_n\}$
...
The dimension of the lattice is $n$. Many cryptographic constructions are
...
\begin{itemize}
	...
	\item \textbf{Versatility:} Lattices have been used to construct a wide
	      ...
	      digital signatures, and importantly for this thesis, vector commitment
	      schemes \cite{LatticeTutorial}.
\end{itemize}

\subsection{Rings instead of matrices}

In lattice-based cryptography, it is more efficient to use polynomial ring
elements, instead of the traditional matrix representations. In these
constructions, operations are performed in the quotient
ring $R_q = \mathbb{Z}_q[x]/(x^n + 1)$, where polynomials are considered
modulo $x^n + 1$. This polynomial modulus induces a negacyclic structure.
Multiplying by $x$ corresponds to a right rotation of coefficient vectors,
with a sign inversion when elements wrap around \cite{Lyubashevsky2013}. The
ring-based approach provides significant advantages over unstructured
lattices, storing a ring element requires only $n$ coefficients rather than
$n^2$ matrix entries, yielding key sizes that are an order of magnitude
smaller \cite{PeikertRingLWE}. Furthermore, polynomial multiplication can be
computed efficiently in $O(n \log n)$ time using the Number Theoretic
Transform (NTT), compared to the $O(n^2)$ complexity of matrix vector
multiplication \cite{Peikert2008}. These efficiency gains, combined with the
worst-case hardness guarantees inherited from Ring-LWE \cite{Peikert2008},
have made ring-based schemes the preferred approach for practical lattice
cryptography.
...
lattice-based commitment scheme. Its security is based on the
hardness of the Shortest Integer Solution (SIS) problem \cite{AjtaiLattices, LatticeTutorial, MSIS}.
...
\begin{itemize}
	...
	\item \textbf{Hiding:} The hiding property is based on the Module Learning
	      With Errors (MLWE) assumption. Informally, the
	      commitment uses randomness, so the published value does not reveal
	      the committed vector. Under MLWE assumptions, the resulting
	      distribution is computationally indistinguishable from uniform to any
	      efficient adversary \cite{LatticesInZKP}.
\end{itemize}
...
$O(\sqrt{N})$. By composing this with the LaBRADOR proof system, Greyhound
...
More recently, Nguyen and Setty proposed Neo \cite{Neo}, a lattice-based
folding scheme for CCS over small prime fields. This is particularly relevant to
the design space of this thesis, because CCS is also the chosen arithmetization
here and the implementation works over a small field.

Neo can be viewed as bringing the HyperNova style of folding into the
lattice-based setting while avoiding some of the overheads of LatticeFold's
cyclotomic-ring formulation. In particular, the paper emphasizes native support
for small fields such as Goldilocks, a single sum-check invocation over an
extension of a small prime field, and a new folding friendly Ajtai commitment
scheme with pay-per-bit commitment costs. These properties make Neo one of the
closest known lattice-based systems to the architecture targeted in this work.

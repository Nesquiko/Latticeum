...
The leading systems in this comparison are all based on STARK-style proving
systems \cite{STARK}, typically combined with FRI commitments \cite{FRI}.
At the same time, these systems also differ in ISA design, trace structure,
memory organization, and recursion strategy, which complicates direct
performance comparison \cite{SoKZkVMs}.
Comparison of currently top 3 ZkVMs can be found in Table \ref{tab:zkvm-comparison}.
...
\begin{table}[h]
	...
	\begin{tabular}{@{}lccc@{}}
		\toprule
		                      & \textbf{Airbender} & \textbf{ZisK} & \textbf{Pico}      \\
		\midrule
		\textbf{Organization} & Matter Labs        & Polygon Labs  & Brevis Network     \\
		\midrule
		\textbf{Hardware}     & 8$\times$RTX 5090  & Unknown       & 16$\times$RTX 5090 \\
		\midrule
		\textbf{Proving time} & 5.9s               & 8.3s          & 9.8s               \\
		\textbf{Proof size}   & $\sim$1MB          & $\sim$260KB   & $\sim$1MB          \\
		\midrule
		\textbf{ISA}          & RV32IM             & RV64IMA       & RV32IM             \\
		\midrule
		\textbf{Proof system} & STARKs             & STARKs        & STARKs             \\
		\textbf{Commitment}   & FRI                & FRI           & FRI                \\
		\midrule
		\textbf{Prime field}  & Mersenne31         & Goldilocks    & KoalaBear/BabyBear \\
		\midrule
		\textbf{Post quantum} & Yes                & Yes           & Yes                \\
		\midrule
		\textbf{Status}       & Production         & Research      & Production         \\
		\bottomrule
	\end{tabular}
	\caption{Comparison of top 3 ZkVMs on EthProofs as of April 2026}

...
They first prove the EVM execution, and then prove knowledge of an EVM proof inside another circuit.
...
$F$ within an augmented function $F'$, where $F$ is the "application" circuit
\cite{Nova, SuperNova, HyperNova}.
...
\cite{Nova, SuperNova, HyperNova}. Figure~\ref{fig:ivc-folding} illustrates this data flow.
...
an open-source Instruction Set Architecture (ISA) \cite{SoKZkVMs, RISCV, RISCVUnprivileged}. RISC-V's
...
for proving not just blockchain applications. Due to this, an EVM written in
...
also AIR-based \cite{OpenvmDesignSpec}, but organizes much of the machine through modular chips, buses,
...
univariate STARK pipeline \cite{SP1Hypercube}. At the same time, these systems also differ in ISA

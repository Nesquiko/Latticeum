...
The leading active systems in this comparison are all based on STARK proving
systems \cite{STARK}, but the internal architecture is no longer uniform.
Some systems use AIR traces with DEEP STARK and FRI, ZisK uses PIL2 state
machines, and the current SP1 Hypercube backend uses a multilinear IOP with
Jagged PCS and Basefold. At the same time, these systems also differ in ISA
...
Table \ref{tab:zkvm-comparison} compares selected zkVMs relevant to this thesis.
Airbender, ZisK, Pico, and OpenVM were active on EthProofs at the time of
writing, while SP1 was listed there as inactive and Nexus zkVM was not listed.
Because these systems expose different final proof artifacts, compressed
recursive proofs in some cases and wrapped SNARKs in others, proof size figures
should be interpreted only as approximate.
...
\begin{table}[h]
	...
	\begin{tabularx}{\textwidth}{@{}l>{\centering\arraybackslash}X>{\centering\arraybackslash}X>{\centering\arraybackslash}X>{\centering\arraybackslash}X>{\centering\arraybackslash}X>{\centering\arraybackslash}X@{}}
		...
		\midrule
		\textbf{Organization}       & Matter Labs        & Polygon Labs      & Brevis             & Succinct      & Nexus              & Axiom               \\
		\midrule
		...
		\midrule
		\textbf{Constraint system}  & AIR-style          & PIL2 SMs          & AIR                & AIR           & AIR                & AIR                 \\
		\textbf{Base zk system}     & DEEP-STARK         & zk-STARK          & STARK              & Hypercube IOP & \shortstack{Circle                       \\ STARK} & \shortstack{STARK +\\ FRI} \\
		\textbf{Wrapper}            & SNARK              & \shortstack{PLONK                                                                                 \\FFLONK}  & Groth16            & \shortstack{Groth16\\PLONK} & n/a                & \shortstack{Halo2\\BN254}         \\
		\bottomrule
	\end{tabularx}
	\caption{Comparison of selected zkVMs relevant}

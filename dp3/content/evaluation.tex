...
as a production-ready ZkVM: the software VM supports a larger instruction set
than the subset currently constrained inside the zkVM, the final Greyhound-based
wrapping proof is not yet integrated, and the full recursive workflow is not
yet proved end-to-end inside in-circuit. The goal of this
chapter is therefore not to claim state-of-the-art performance, but to compare
the prototype with existing ZkVMs, to explain the observed differences, and to
...
A benchmark was prepared with two \texttt{fibonacci} guest variants. The first,
non-optimized variant computes the 100th Fibonacci number inside the guest
itself. Its compiled binary contains around 60 instructions. That full runtime
trace is too long to prove and benchmark comfortably in the current prototype.
For the reported measurements, the optimized guest variant was therefore used
instead, where the compiler effectively materializes the wrapped 32-bit 100th
Fibonacci value as a constant and the program only writes it to memory. This,
not a Fibonacci calculation, but nevertheless enough of a program,
keeps the benchmark at a reasonable scale while still exercising the proof pipeline.

This simple benchmark program execution in the prototype resulted in:
...
\begin{table}[h]
	...
	\begin{tabular}{|l|c|c|}
		...
		\hline
	\end{tabular}
	\caption{Comparison of proof generation time and peak RAM usage for the benchmark program.}
	\label{tab:zkvm-benchmarks-v2}
\end{table}

\noindent\textit{* The Nexus zkVM 3.0 result reflects the documented
	proof flow from the public SDK quick-start. The public
	Nexus documentation consulted for this thesis does not provide a concrete
	instructions for the full recursive/wrapped proving pipeline, so that pipeline was
	not run.}
...
For the benchmark, each ZkVM was run according to its own official
setup guide. This thesis followed the instructions from the setup guide in this
document's appendix. Pico followed its quick-start guide\footnote{\url{https://pico-docs.brevis.network/getting-started/quick-start.html}},
SP1 followed the official quickstart\footnote{\url{https://docs.succinct.xyz/docs/sp1/getting-started/quickstart}},
ZKsync Airbender followed its guide\footnote{\url{https://github.com/matter-labs/zksync-airbender/blob/main/docs/overview.md}},
and Nexus zkVM 3.0 followed its SDK quick-start\footnote{\url{https://docs.nexus.xyz/zkvm/development/sdk-quick-start}}.
To keep the comparison practical and reproducible, all systems were exercised on
the same optimized \texttt{fibonacci} guest.

To complement the small benchmark, a longer experiment was also run
with the \texttt{evm} guest based on \texttt{REVM}. The guest program proved
a Etherum block 24740084. This run continued for
...
usage remained around 13.7 GB. Unfortunately, the run didn't complete, because
of a bug in the implementation, where the zkVM panicked due to incorrect logic
of a instruction handler.
This is far from production-level
...
increase or failing to fold inside the IVC chain.
...
This observation is supported by profiling of the benchmark proving run. The
...
of the witness rather than by the small benchmark program itself.
...
\textit{How does the performance of a lattice-based folding scheme compare to established approaches of proving a general-purpose VM execution?}
...
\textbf{Answer:} Yes, the results are encouraging.

From the design perspective, LatticeFold controls witness growth through its
decomposition layer, so the witness norm is kept within the protocol bounds even
as the execution chain grows. That means long computations are not expected to
fail simply because the witness becomes too large.

From the implementation perspective, the long-running
\texttt{REVM}-based EVM experiment, although that run eventually stopped because
of a bug in an instruction handler, it kept progressing for more than 9 hours
and the IVC chain continued to fold successfully. This is a positive sign that
the witness norm remained low enough for LatticeFold to keep operating. If the
norm had grown beyond the allowed bound, the decomposition and folding checks
would have failed and the IVC chain would not have been able to continue.

The prototype does not yet
complete the full long-running EVM experiment, but the observed behavior shows
that latticefold-based norm control is working and that long executions are not
being blocked by witness blow-up.

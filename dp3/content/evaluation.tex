...
itself; its compiled binary contains 35 static instructions in the text section,
including 27 instructions in \\texttt{main}. That full runtime trace is too long
to prove and benchmark comfortably in the current prototype. For the reported
measurements, the optimized guest variant was therefore used instead, where the
compiler effectively materializes the wrapped 32-bit 100th Fibonacci value as a
constant and the program only writes it to memory. This keeps the benchmark at a
reasonable scale while still exercising the proof pipeline.

The optimized Fibonacci program execution in the prototype resulted in:

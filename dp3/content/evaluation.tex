...
A simple benchmark was conducted using the compiled \\texttt{fibonacci} guest.
In the optimized guest binary, the wrapped 32-bit value of the 100th Fibonacci
number is effectively hardcoded by the compiler rather than recomputed through
the full iterative loop at runtime. This means the benchmark does not measure a
real 100-step Fibonacci computation, but only the execution of the optimized
guest that materializes the final constant and writes it to memory. Even so,
this is still sufficient as a small reproducible benchmark for comparing the
prototype against other ZkVMs.

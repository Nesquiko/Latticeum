...
post-quantum non hash-based ZkVMs and establishes the specific scientific questions
and objectives that guide the subsequent solution design, implementation and evaluation.
...
Some currently deployed ZkEVMs and ZkVMs rely on cryptographic hardness
assumptions, specifically the Discrete Logarithm Problem in elliptic curve
groups, that are vulnerable to attacks by sufficiently powerful quantum
computers. Majority of ZkVMs use hash-based STARKs \cite{STARK}, which offer a post-quantum
alternative, but they incur significant costs in terms of proof size or
...
literature and open-source ecosystem, \textbf{there is currently no practical
	solution of a general-purpose RISC-V ZkVM built entirely upon lattice-based primitives.}
...
This difficulty is reinforced by the fact that existing ZkVMs already span a
highly heterogeneous design space across the ISA, VM, and proving layers
\cite{SoKZkVMs}.
...
\begin{enumerate}
	...
	\item \textbf{Performance trade-offs:} \textit{How does the performance of
		      a lattice-based folding scheme compare to established approaches of proving
		      a general-purpose VM execution?}

...
selected cryptographic building blocks. From the perspective of zkVM
architecture, these choices belong mainly to the proving layer, which interacts
closely with the ISA and execution design decisions surveyed in
\cite{SoKZkVMs}.
...
step $i-1$. RISC-V execution traces of unbounded length align with this model.
...
SNARK verification. The resulting recursive proof can then be compressed by a
Spartan-style SNARK \cite{Spartan}.
...
S-Box, which is exponentiation to the 7th degree \cite{Poseidon, Poseidon2}). A CCS structure $S$
consists of:
...
LatticeFold \cite{LatticeFold} can be thought of as a HyperNova \cite{HyperNova},
...
This wrapper is constructed by combining the \textbf{SuperSpartan} \cite{Spartan, HyperNova}
...
allowing instantiation of the Sum-Check protocol \cite{SumcheckArticle} over the lattice
...
\begin{enumerate}
	...
	\item \textbf{$(a, t)$-ordering proof}: verifies that $tr'$ is correctly sorted
	      ...
	      and conditional uniqueness sub-proofs.

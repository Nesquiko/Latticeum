...
Following recent zkVM systematization work \cite{SoKZkVMs}, this design can be
understood as a composition of an ISA layer, an execution layer that produces
traces, and a proving layer that arithmetizes and proves them.
...
\begin{enumerate}
	...
	\item \textbf{Execution and trace generation:} The emulator executes the
	      ...
	      \begin{itemize}
		      ...
		      \item \textbf{Side effects:} A set of flags indicating branching
		            behavior (jump targets), and memory operations (address,
		            value, and read/write flags).
	      \end{itemize}

	\item \textbf{Arithmetization and witness generation:} The raw data are
	...
	\item \textbf{The augmented circuit $F'$:} To achieve
	      ...
	      \begin{enumerate}
		      \item \textbf{IVC integrity checks:} It recalculates the commitment to
		            ...
		            \cite{Nova}, SuperNova \cite{SuperNova}, and HyperNova \cite{HyperNova}.
	...
	\item \textbf{Memory logging:} If the instruction execution triggers a
	      ...
	      the execution concludes \cite{MemoryAccessInSnarks}.

	\item \textbf{Final wrapping:} Once the program halts, the system holds a
	      single folded instance-witness pair and final folding proof.
	      These could be used to prove the execution, but they are not succinct. To produce a
	      succinct proof suitable for blockchain verification, these large
	      lattice-based artifacts are wrapped in a final SNARK as depicted in Figure \ref{fig:snark_wrap}.
	      This wrapper consist of the SuperSpartan IOP \cite{HyperNova} to
	      prove the validity of the folded CCS instance, combined with the
	      Greyhound \cite{Greyhound} PCS to ensure post-quantum security and
	      succinct verification.
\end{enumerate}

\begin{tikzFigure}
	\begin{tikzpicture}
		\node[dataNode, xshift=-2.4cm] (rust) {Rust source};
		...
		\node[dataNode, below=1cm of vm] (witness) {Witness $w_i$};
		\draw[arrow] (vm) -- node[midway, right] {Exec instruction $i$} (witness);

		\node[processNode, below left=1.5cm and 0.7cm of witness] (stepCircuit) {Step circuit $F$\\ (RISC-V logic)};
		...
		\node[dataNode, below=1cm of augCircuit] (memLog) {Memory\\operations log};
		\draw[arrow] (stepCircuit.west) -- ++(-1.5,-1) -- ++(0,-1.5) -- (memLog.west) node[midway, left] {\scriptsize Memory logs};

		\coordinate[left=0.8cm of ivcProofPrev] (dotsStart);
		\coordinate[right=0.8cm of ivcProofNext] (dotsEnd);
		\path (outerBox.south east) ++(0.2cm,-1.4cm) coordinate (repeatRight);
		\path (outerBox.south west) ++(-0.2cm,-1.4cm) coordinate (repeatLeft);
		\path (memLog.south west) ++(-0.3cm,-0.35cm) coordinate (repeatUnderLogLeft);
		\path (memLog.south east) ++(0.3cm,-0.35cm) coordinate (repeatUnderLogRight);
		\path (repeatUnderLogLeft) -- (repeatUnderLogRight) coordinate[midway] (repeatMid);
		\draw[arrow, dotted] (dotsStart) -- (accPrev);
		\draw[arrow, dotted] (dotsStart) -- (ivcHPrev);
		\draw[arrow, dotted] (dotsStart) -- (ivcProofPrev);
		\draw[arrow, dotted] (accNext) -- (dotsEnd);
		\draw[arrow, dotted] (ivcHNext) -- (dotsEnd);
		\draw[arrow, dotted] (ivcProofNext) -- (dotsEnd);
		\draw[arrow, dotted] (dotsEnd) to[out=-20, in=0] (repeatRight) -- (repeatUnderLogRight) -- (repeatUnderLogLeft) -- (repeatLeft) to[out=180, in=-160] (dotsStart);

		\node[wrapperNode, below=2.9cm of memLog] (wrapper) {Wrapper SNARK\\(SuperSpartan + Greyhound)};
		\node[dataNode, above=0.6cm of wrapper, xshift=-2.8cm] (accFinal) {Accumulator\\$U_{final}$};
		\node[dataNode, above=0.6cm of wrapper] (finalProof) {Final folding proof\\$\pi_{final\ fold}$};
		\node[dataNode, above=0.6cm of wrapper, xshift=2.8cm] (memLogFinal) {Memory\\operations log};
		\node[dataNode, right=1.8cm of wrapper] (proof) {Final succinct proof\\$\pi_{final}$};
		\draw[arrow, dotted] (repeatUnderLogRight) to[out=-80, in=90] node[midway, right] {After last step} (finalProof.north);
		\draw[arrow] (accFinal.south) -- (wrapper.north);
		\draw[arrow] (finalProof.south) -- (wrapper.north);
		\draw[arrow] (memLogFinal.south) -- (wrapper.north);

...
correct folding of previous steps \cite{Nova, SuperNova}.
...
mathematical correctness of the folding process without re-executing the prior computation.
...
The execution of one IVC step concludes by generating the artifacts required for the
...
\subsection{IVC verification}

After the final IVC chain terminates, the output is final witness-instance pair
and a proof of last folding. These artifacts would be enough for proving
that there exists a valid execution trace of a program $P$, staring from public
initial state $z_0$, ending in public final state $z_n$, such that every
instruction transition follows the selected RISC-V ISA, every memory
operation is consistent with read-over-write semantics, and every
folded accumulator is valid under LatticeFold. However, they are not succinct,
so they must be wrapped in order to produce a succinct proof.
